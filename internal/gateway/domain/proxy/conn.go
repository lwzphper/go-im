...
func Handle(c *gin.Context) {
	...
	if c.Query(config.GatewayAuthKey) != config.GatewayAuthVal {
		...
		wsConn.WriteMessage(websocket.TextMessage, connect.MarshalOutput(connect.MethodServiceNotice, "无权操作", 0))
		return
	}
	...
	for {
		...
		var data = new(connect.QueueMsgData)
		err = json.Unmarshal(message, data)
		if err != nil {
			logger.Infof("IMServer消息格式有误：%s", string(message))
			_ = wsConn.WriteMessage(websocket.TextMessage, connect.MarshalOutput(connect.MethodServiceNotice, "消息格式有误", 0))
			continue
		}
